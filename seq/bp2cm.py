...
parser.add_argument(	"positions", type = readable_or_stdin_handle,
...
mapfile = csv.reader(args.map, delimiter = "\t")
for row in mapfile:
	rawmap[row[0]].append( (float(row[2]), int(row[3])) )
...
posfile = csv.reader(args.positions, delimiter = "\t")
for row in posfile:
	...
	if not row[0] in bp:
		...
		print row[0], row[1], interp_cm, row[2]
