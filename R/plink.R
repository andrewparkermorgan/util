...
## NB: conversion function (matrix to tped) remains in <geno.R>
...
pca.selection.scan <- function(prefix, K = 5, prune = FALSE, ...) {
	
	prefix.new <- prefix
	if (prune) {
		message("Doing LD-pruning...")
		prefix.new <- prune.plink(prefix, ...)
	}
	
	message("Performing PCA with plink...")
	pcs <- pca.plink(prefix.new, K = K, by = "var")
	
	message("Adding marker data...")
	map <- read.map(prefix.new)
	pcs <- merge(pcs[ ,-1 ], map)
	
	message("Computing test statistics and p-values...")
	rez <- pcs[ ,c("chr","marker","cM","pos") ]
	for (col in paste0("PC", seq_len(K))) {
		rez[ ,col ] <- -1*log10(1 - pchisq(pcs[ ,col ]^2, df = 1))
	}
...
## utility function to dump markers currently in use in a plink dataset
dump.markers <- function(prefix, flags = "", ...) {
	
	if (!inherits(prefix, "plink"))
		stop("Not convinced the input is a pointer to a plink fileset.")
	
	ff <- tempfile()
	message(paste0("Dumping to <", paste0(ff, ".snplist"), "> ..."))
	cmd <- paste0("plink --bfile '", prefix, "' ", flags, " --write-snplist --out ", ff)
	rez <- system(cmd, intern = FALSE)
	return( read.table(paste0(ff, ".snplist"), stringsAsFactors = FALSE)[,1] )
